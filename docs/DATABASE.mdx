...
The web application includes a database status dashboard at `/db` that provides:

- **Connection Status** - Real-time connection health with latency monitoring
- **Database Metadata** - Host, port, and database name (credentials hidden)
- **Tables Overview** - List of available tables with row counts
- **Quick Commands** - Common Prisma commands for easy reference
- **Setup Guide** - Full documentation available at `/db/setup`
...
2. Navigate to [http://localhost:3000/db](http://localhost:3000/db)

### Features

| Route       | Description                    |
| ----------- | ------------------------------ |
| `/db`       | Main status dashboard          |
| `/db/setup` | Full setup documentation (MDX) |

The dashboard also provides a direct link to Prisma Studio (localhost:5555) when running.
...
- [ ] Check connection status at `/db` in the web app
